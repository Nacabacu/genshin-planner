...
(async () => {
  ...
  const data: Weapon | null = await page.$eval('body', (element) => {
    ...
    const rarity = element
      .querySelector<HTMLImageElement>('[data-source="rarity"] > div > img')
      ?.title.toLowerCase();
    const table =
      element.querySelector<HTMLTableElement>('.wikitable >tbody')?.children;
    ...
    for (let index = 0; index < table.length; index += 1) {
      const item = table[index].querySelectorAll(
        '.mw-collapsible-content > .card_container',
      );
      ...
      for (let itemIndex = 0; itemIndex < item.length; itemIndex += 1) {
        ...
        const name = card
          .querySelector<HTMLAnchorElement>('.card_image > a')
          ?.title.toLowerCase();
        const quantityString =
          card.querySelector<HTMLSpanElement>('.card_font')?.innerText;
